...
void UMesh:: FaceMatch(const ExaMesh* const pEM,
	...
	for(emInt iPart=0 ; iPart<parts.size(); iPart++){
		...
		for (auto tri : partBdryTris){
			...
			addUniquely(SetTriPartbdry,tripart); 	
			UUM->addBdryTri(conn);
		}
		...
		for (auto quad : partBdryQuads) {
			...
			quad.getCorner(1),quad.getCorner(2),
			quad.getCorner(3)}; 
			QuadFaceVerts quadpart(numDivs,conn,global,iPart);	
			addUniquely(SetQuadPartbdry,quadpart);  		
			UUM->addBdryQuad(conn);
		}

	}

	for(auto itr=SetTriPartbdry.begin(); itr!=SetTriPartbdry.end();itr++){
		auto next= itr++; 
		emInt v0Global= next->getGlobalCorner(0);
		emInt v1Global= next->getGlobalCorner(1);
		emInt v2Global= next->getGlobalCorner(2); 

		emInt v0Local= next->getCorner(0);
		emInt v1Local= next->getCorner(1);
		emInt v2Local= next->getCorner(2); 
		...
		emInt v0Local_= itr->getCorner(0);
		emInt v1Local_= itr->getCorner(1);
		emInt v2Local_= itr->getCorner(2); 
		...
		if(v0SortedGlobal_==v0SortedGlobal &&
			...
			v2SortedGlobal==v2SortedGlobal_ ){
				emInt global[3] = {v0Global,v1Global,v2Global}; 
				emInt global_[3]= {v0Global_,v1Global_,v2Global_}; 
				emInt local [3] = {v0Local,v1Local,v2Local}; 
				emInt local_ [3]= {v0Local_,v1Local_,v2Local_};
				TriFaceVerts tripart(numDivs,local,global,partid,partid_);
				TriFaceVerts tripart_(numDivs,local_,global_,partid_,partid);
				tris[partid].push_back(tripart); 
				tris[partid_].push_back(tripart_); 
	...
	for(auto itr=SetQuadPartbdry.begin(); 
	itr!=SetQuadPartbdry.end();itr++){
		auto next= itr++; 
		emInt v0Global= next->getGlobalCorner(0);
		emInt v1Global= next->getGlobalCorner(1);
		emInt v2Global= next->getGlobalCorner(2); 
		...
		emInt v1Local= next->getCorner(1);
		emInt v2Local= next->getCorner(2); 
		emInt v3Local= next->getCorner(3); 
		...
		emInt v1Local_= itr->getCorner(1);
		emInt v2Local_= itr->getCorner(2); 
		emInt v3Local_= itr->getCorner(3); 
		...
		if(v0SortedGlobal_==v0SortedGlobal &&
			...
			v3SortedGlobal==v3SortedGlobal_){
				...
				emInt local [4] = {v0Local,v1Local,v2Local,v3Local}; 
				emInt local_ [4]= {v0Local_,v1Local_,v2Local_,v3Local_};
				QuadFaceVerts quadpart(numDivs,local,global,partid,partid_);
				QuadFaceVerts quadpart_(numDivs,local_,global_,partid_,partid);
