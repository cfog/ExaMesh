...
#include <numeric>
std::string 
writeParallelJobTemplate 
(int nodes, int tasks, std::string jobName, int numDivs, std::string cgnsOrUmesh, std::string meshName)
{
   // std::string fullName = "Jobs/"+ jobName+".sh"; 
    ...
    out<<"srun ./refine -"<<cgnsOrUmesh<< " TestCases/"<<meshName<<" -n "<<numDivs<<" -q"<<std::endl;
    return fullName; 
}
void 
writeSerialJobTemplate 
(std::string jobName, int numDivs, std::string cgnsOrUmesh, std::string meshName)
{
    ...
    out<<"#SBATCH --mem=0"<<std::endl;
    out<<"./refine -"<<cgnsOrUmesh<< " TestCases/"<<meshName<<" -n "<<numDivs<<std::endl;
...
int main(int argc, char* const argv[]) 
{
    ...
    for(auto i=0 ; i<array.size(); i++)
    {
        ...
        for (auto k=0 ; k<nProcessors.size(); k++)
        {
            ...
            auto jbName = writeParallelJobTemplate(nodes,task,jobName,array[i],cgnsorUmesh,fileName);
