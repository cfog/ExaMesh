...
void ParallelTester::setVecpart(const vecPart &inVecPart) {
	m_vecPart.assign(inVecPart.begin(), inVecPart.end());
}
void ParallelTester::setvecCellPartData(
		const vecCellPartData &inVecCellPartData) {
	m_vecPartData.assign(inVecCellPartData.begin(), inVecCellPartData.end());
}
void ParallelTester::testVecPart(const int rank, const Part &inPart) const {
	assert(m_vecPart[rank] == inPart);
}
void ParallelTester::testVecCellPartData(
		const vecCellPartData &inVecCellPartData) const {
	assert(m_vecPartData.size() == inVecCellPartData.size());
	// since the data is broadcasted no need for rank
	assert(
			std::equal(m_vecPartData.begin(), m_vecPartData.end(),
					inVecCellPartData.begin()));

}
;
void ParallelTester::setInputTri(const vecHashTri &inTris) {
...
void ParallelTester::testInputTri(const int rank, const hashTri &inTri) const {
...
void ParallelTester::setMatchedTris(const vecMatchedTris &inMatchedTris) {
	m_matchedTrisAllParts = inMatchedTris;
}
void ParallelTester::setMatchedQuads(const vecMatchedQuads &inMatchedQuads) {
	m_matchedQuadsAllParts = inMatchedQuads;
}

void ParallelTester::testMatchedTris(
		const TableTri2TableIndex2Index &matchedTris, const int rank) const {
...
void ParallelTester::testMatchedQuads(
		const TableQuad2TableIndex2Index &matchedQuads, const int rank) const {
